# Concise UI Rules (MUST / SHOULD / NEVER)

Editable decision rules used by the `ui-audit-laws` skill.
Upstream: [vercel-labs/web-interface-guidelines AGENTS.md](https://github.com/vercel-labs/web-interface-guidelines/blob/main/AGENTS.md).

**How to maintain this file**
- Edit freely — this local copy is the audit source of truth, not the upstream repo.
- Keep the MUST / SHOULD / NEVER verbs; severity in audits maps as: MUST/NEVER → blocking, SHOULD → major (or minor if polish-only).
- When adding a rule, put it under the matching section and use the same `MUST:` / `SHOULD:` / `NEVER:` prefix.
- Optional: sync from upstream with `curl -fsSL https://raw.githubusercontent.com/vercel-labs/web-interface-guidelines/main/AGENTS.md` then re-apply this header.
