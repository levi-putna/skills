...
Binding checks for scene planning (Gate 2), shared components (Gate 3), and
every rendered still (Gate 5 self-audit). Adapted from a generative-video
...
flag *what* is shared (Gate 3, step 1); the sharing itself lives in code.
...
- One teaching idea per scene (this is the scene's `keyPoint` from Gate 2).
...
theme defined at Gate 3, applied consistently across scenes.
...
## Self-audit before presenting a scene (Gate 5)
